...
function GameCanvas({ stage, angle, power, guide, firingKey, availableWater, onGoal, onSpent, onPhysicsStatus, resetKey }: { stage: Stage; angle: number; power: number; guide: boolean; firingKey: number; availableWater: number; onGoal: () => void; onSpent: (count: number) => void; onPhysicsStatus: (status: PhysicsStatus) => void; resetKey: number }) {
  ...
  const availableWaterRef = useRef(availableWater);

  useEffect(() => { angleRef.current = angle; }, [angle]);
  useEffect(() => { stageRef.current = stage; }, [stage]);
  useEffect(() => { availableWaterRef.current = availableWater; }, [availableWater]);
  ...
  useEffect(() => { if (firingKey > 0) shots.current = { remaining: Math.min(WORLD.shotCount, availableWaterRef.current), nextAt: performance.now(), power }; }, [firingKey, power]);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const moving = s.obstacles.map((o, i) => ({ ...o, x: o.x + (o.move?.axis === "x" ? Math.sin(now * .001 * o.move.speed + (o.move.phase ?? i)) * o.move.distance : 0), y: o.y + (o.move?.axis === "y" ? Math.sin(now * .001 * o.move.speed + (o.move.phase ?? i)) * o.move.distance : 0) }));
      const currentGoal = { ...s.goal, x: s.goal.x + (s.goal.move?.axis === "x" ? Math.sin(now * .001 * s.goal.move.speed + (s.goal.move.phase ?? 0)) * s.goal.move.distance : 0), y: s.goal.y + (s.goal.move?.axis === "y" ? Math.sin(now * .001 * s.goal.move.speed + (s.goal.move.phase ?? 0)) * s.goal.move.distance : 0) };
      const zones = getGoalZones(currentGoal);
      particles.current.forEach(p => {
        if (!p.active) return;
        if (p.hasEnteredGoal) {
          p.vx *= .84; p.vy = Math.max(45, p.vy + s.gravity * dt * .35); p.x += (currentGoal.x + 24 - p.x) * Math.min(1, dt * 8); p.y += p.vy * dt;
          if (isPointInside(p, zones.disposal) || p.y >= zones.disposal.bottom) p.active = false;
          return;
        }
        p.vy += s.gravity * dt; p.x += p.vx * dt; p.y += p.vy * dt;
        if (shouldCountGoalParticle(p, currentGoal)) { p.hasEnteredGoal = true; onGoal(); return; }
        let absorbed = false;
        for (const o of moving) {
          if (p.x <= o.x || p.x >= o.x + o.w || p.y <= o.y || p.y >= o.y + o.h) continue;
          if (o.kind === "slopeUp" || o.kind === "slopeDown") {
            const t = (p.x - o.x) / o.w;
            const surfaceY = o.kind === "slopeUp" ? o.y + o.h * (1 - t) : o.y + o.h * t;
            if (p.y < surfaceY - 8 || p.y > surfaceY + 12 || p.vy < 0) continue;
            const tangentX = o.w; const tangentY = o.kind === "slopeUp" ? -o.h : o.h;
            const length = Math.hypot(tangentX, tangentY); const tx = tangentX / length; const ty = tangentY / length;
            const along = Math.max(75, p.vx * tx + p.vy * ty); p.x += tx * 4; p.y = surfaceY - 4; p.vx = tx * along * .94; p.vy = ty * along * .94; continue;
          }
          if (o.kind === "bounce") {
        ...
        if (absorbed || p.y >= WORLD.groundY || p.x < -10 || p.x > WORLD.width + 10 || p.y < -20 || now - p.born > WORLD.particleLife) p.active = false;
      });
      let inGoalArea = 0; let approachingGoal = 0;
      particles.current.forEach(p => { if (!p.active) return; if (p.hasEnteredGoal || isPointInside(p, zones.entrance) || isPointInside(p, zones.internal)) inGoalArea++; if (isApproachingGoal(p, currentGoal)) approachingGoal++; });
      ...
      moving.forEach(o => { ctx.fillStyle = o.kind === "bounce" || o.kind === "slopeUp" || o.kind === "slopeDown" ? "#ffcc45" : "#7b5261"; if (o.kind === "slopeUp" || o.kind === "slopeDown") { ctx.beginPath(); ctx.moveTo(o.x,o.y+o.h); ctx.lineTo(o.kind === "slopeUp" ? o.x+o.w : o.x,o.y); ctx.lineTo(o.x+o.w,o.y+o.h); ctx.closePath(); ctx.fill(); } else { ctx.fillRect(o.x, o.y, o.w, o.h); ctx.fillStyle = o.kind === "bounce" ? "#fff08a" : "#a97b85"; ctx.fillRect(o.x + 4, o.y + 4, Math.max(0,o.w - 8), 6); } });
      const g = currentGoal; const gs = g.size ?? 1; const gcx = g.x + 24; ctx.fillStyle = "#8a9ba9"; ctx.fillRect(gcx - 32*gs, g.y + 13*gs, 64*gs, 44*gs); ctx.fillStyle = "#d8e3e8"; ctx.fillRect(gcx - 28*gs, g.y + 17*gs, 56*gs, 36*gs); ctx.fillStyle = "#14242c"; ctx.beginPath(); ctx.ellipse(gcx, g.y + 17*gs, 28*gs, 12*gs, 0, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = "#fff"; ctx.font = "800 13px sans-serif"; ctx.textAlign = "center"; ctx.fillText("GOAL", gcx, g.y - 7);
...
export function WaterShotGame() {
  ...
  const startStage=useCallback((index:number)=>{ const safeIndex=Math.max(0,Math.min(STAGES.length-1,index)); const s=STAGES[safeIndex]; clearFinalized.current=false; lastGoalCountChangedAt.current=0; setStageIndex(safeIndex); setAngle(s.gun.angle); setWater(s.water); setGoal(0); setShotCount(0); setGameState("playing"); setPhysicsStatus({firing:false,inGoalArea:0,approachingGoal:0}); setPower(0); setCharging(false); setFiringKey(0); setResult(null); setResetKey(k=>k+1); setScreen("game"); },[]);
  ...
  return <main className="app-shell">
    ...
    <section className="phone-frame">
      ...
      {screen==="game"&&<div className="game-screen" onPointerMove={e=>{if(dragStart.current){const next=dragStart.current.angle+(dragStart.current.y-e.clientY)*.22;setAngle(Math.max(5,Math.min(85,next)))}}} onPointerUp={()=>dragStart.current=null}>
        ...
        {result&&<div className="modal-backdrop"><div className={stage.id===STAGES.length&&result.status==="clear"?"result-card final-clear":"result-card"}><small>{result.status==="clear"?(stage.id===STAGES.length?"ALL STAGES COMPLETE":"STAGE COMPLETE"):"TRY AGAIN"}</small><h2 data-text={resultTitle}>{resultTitle}</h2><div className="result-stars">{result.status==="clear"?stars(result.stars):"💧"}</div><p>{result.status==="clear"?`${stage.name} クリア！`:stage.hint}</p>{result.status==="clear"&&stage.id<STAGES.length&&<button className="primary" onClick={()=>startStage(stageIndex+1)}>次のステージ ▶</button>}<button className="secondary" onClick={()=>startStage(stageIndex)}>↻ リトライ</button><button className="text-button" onClick={()=>setScreen("stages")}>ステージ選択へ</button></div></div>}
      </div>}
    </section>
  </main>;
}
